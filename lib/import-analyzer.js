...
function analyzeImports(targetDir, options = { 
  ...
  const jsonReportPath = path.join(options.outputDir, `${options.reportNamePrefix || 'import-analysis-report'}.json`);
  fs.writeFileSync(jsonReportPath, JSON.stringify(jsonReport, null, 2));
  
  // Generate HTML report if requested
  if (options.generateHtml !== false) {
    const htmlReport = generateHtmlReport(jsonReport);
    const reportPath = path.join(options.outputDir, `${options.reportNamePrefix || 'import-analysis-report'}.html`);
