...
function resolveAliases(importPath, targetDir) {
  // Try to find .importcheckerrc.json
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.warn(`Warning: Failed to parse .importcheckerrc.json`);
      return importPath;
    }
  } else {
    return importPath;
  ...
  const { aliases, baseUrl = "." } = config;
  const baseDirectory = path.join(process.cwd(), baseUrl);
  ...
  for (const [alias, aliasPath] of Object.entries(aliases)) {
    ...
    if (importPath.startsWith(aliasPattern)) {
      ...
      const resolvedPath = importPath.replace(aliasPattern, actualPath);
      return path.relative(
        path.dirname(targetDir),
        path.join(baseDirectory, resolvedPath),
      );
    }
  }
    ...
    <body class="bg-dark-bg text-gray-200 min-h-screen">
      <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
          <h1 class="text-3xl font-bold text-white">Import Analysis Report</h1>
          <button 
      ...
      <script>
        ...
        function downloadJson() {
          ...
          linkElement.setAttribute('download', 'import-analysis-report.json');
    ...
    try {
      ...
      function visitNode(node) {
        if (node.type === "ImportDeclaration") {
          ...
          // Resolve any aliases in the import path
          const importPath = resolveAliases(rawImportPath, file);
          ...
          if (importPath.startsWith(".")) {
            ...
            // Only consider imports if they are within the parent directory
            // and from a sibling directory
            if (
              !relativeToParent.startsWith("..") &&
              siblingDirs.includes(importDirName)
            ) {
              foundImports.add(
                `${relativePath} imports from sibling directory '${importDirName}': ${rawImportPath} ${importedItemsStr}`,
              );
            }
          } else {
            // Handle absolute imports and aliased imports
            // First check if it's a sibling directory import after alias resolution
            const absoluteImportPath = path.resolve(process.cwd(), importPath);
            const relativeToParent = path.relative(
              parentDir,
              absoluteImportPath,
            );
